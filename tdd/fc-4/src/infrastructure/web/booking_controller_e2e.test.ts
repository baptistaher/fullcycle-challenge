...
beforeAll(async () => {
  ...
  app.post("/bookings", (req, rest, next) => {
    bookingController.createBooking(req, rest).catch((err) => next(err));
  });
...
describe("bookingController", () => {
  beforeAll(async () => {
    ...
    await propertyRepo.save({
      ...
      description: "Descrição da propriedade",
      maxGuests: 5,
  ...
  it("should create a booking with success", async () => {
  ...
  it("should return 300 when  try to create a booking with  invalid  start date ", async () => {
    const response = await request(app).post("/bookings").send({
      propertyId: "1",
      guestId: "1",
  ...
  it("should return 300 when  try to create a booking with  invalid  end date ", async () => {
    const response = await request(app).post("/bookings").send({
      propertyId: "1",
      guestId: "1",
      startDate: "2024-12-20",
  ...
  it("should return  400 when try to create a booking with invalid guest number", async () => {
    const response = await request(app).post("/bookings").send({
      propertyId: "1",
      guestId: "1",
      startDate: "2024-12-20",
  ...
  it("should return 400 when try to create a booking with invalid property id", async () => {
    const response = await request(app).post("/bookings").send({
  ...
  it("should cancel a booking", async () => {
    ...
    it("should throw an error when canceling with invalid booking id", async () => {
